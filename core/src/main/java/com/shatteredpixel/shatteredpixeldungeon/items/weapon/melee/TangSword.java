...
import com.shatteredpixel.shatteredpixeldungeon.actors.buffs.Armorbroken;
...
public class TangSword extends MeleeWeapon {

	{
		image = ItemSpriteSheet.TANGSWORD;
		hitSound = Assets.Sounds.HIT_SLASH;
		hitSoundPitch = 1.2f;

		tier = 4;
	}
	public static int addBrokenlevel = 0;

	@Override
	public int max(int lvl) {
		return  4*(tier+1) +    //20 base, down from 25	最大伤害从25降低到20
	...
	protected void duelistAbility(Hero hero, Integer target) {
		TangSword.cleaveAbility(hero, target, 1.27f, this);
	}
	...
	public int proc( Char attacker, Char defender, int damage ) {
		...
		//挂增伤标记
		if ( defender.buffs( Armorbroken.class ) != null ){
			for ( Buff buff : defender.buffs() ){
				addBrokenlevel = level() + 1;
			}
		}else{
			Buff.affect( defender, Armorbroken.class );
		}

		//伤害增加
		for ( Buff buff : defender.buffs()){
			if( buff instanceof Armorbroken){
				damage *= Math.pow ( 1.15f,level() );
			}
		}
