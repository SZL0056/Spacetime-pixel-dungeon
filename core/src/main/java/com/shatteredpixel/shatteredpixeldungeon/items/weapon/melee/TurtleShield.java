...
public class TurtleShield extends MeleeWeapon {

	{
		image = ItemSpriteSheet.TURTLESHIELD;
	...
	public int max(int lvl) {
		return  Math.round(2f*(tier+1)+2) +   //10 base, down from 15
				lvl*(tier);               //+2 per level, down from +4
	}

	@Override
	public int defenseFactor( Char owner ) {
		return 2+buffedLvl();               //4 extra defence, plus 1 per level
	}
	...
	protected void duelistAbility(Hero hero, Integer target) {
		TurtleShield.guardAbility(hero, 5, this);
	}
